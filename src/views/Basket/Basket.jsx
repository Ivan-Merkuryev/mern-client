...
function Basket() {
  ...
  const [noGoods, setNoGoods] = useState();
  //Modal
  ...
  const order = async () => {
    try {
      const res = await axios.post("clearBasketAndAddToOrders");
      // alert("");
      setModalActive(true);
  ...
  const ordersList = () => {
    return (
      <div className="orders">
        <div>
          {orders.length > 0 ? <h3>Заказы</h3> : null}
          {orders.map((el) => (
